...
session_start();
 ...
 if(isset($_REQUEST['passupdate'])){
  if(($_REQUEST['aPassword'] == "")){
   ...
   $passmsg = '<div class="alert alert-warning col-sm-12 ml-1 mt-2" role="alert"> Fill All Fileds </div>';
  } else {
    ...
    if($result->num_rows == 1){
     ...
     $sql = "UPDATE adminlogin_tb SET a_password = '$aPass' WHERE a_email = '$aEmail'";
      if($conn->query($sql) == TRUE){
       ...
       $passmsg = '<div class="alert alert-success col-sm-12 ml-1 mt-2" role="alert"> Updated Successfully </div>';
      } else {
       // below msg display on form submit failed
       $passmsg = '<div class="alert alert-danger col-sm-12 ml-1 mt-2" role="alert"> Unable to Update </div>';
      }
    }
   }
}
